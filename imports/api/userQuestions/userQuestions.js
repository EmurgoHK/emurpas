...
if (Meteor.isServer) {
  Meteor.publish("userQuestions.teamMembers", function(projectId) {
    const project = ProjectQuestions.findOne(projectId);
    if (!project) return undefined;

    return UserQuestions.find(
      { createdBy: { $in: project.team_members } },
      { fields: { _id: true, createdBy: true } }
    );
  });
...
UserQuestions.schema = new SimpleSchema(
  {
    ...
    teamDescription: {
      ...
      label:
        "Describe the other people on your team and why you want to work with them."
    },
    averageTeamWorkday: {
      type: String,
      max: 300,
    ...
    relatedPastProjects: {
      ...
      label:
        "What other past projects have you worked on related to the one now?"
    },
    projectGoals: {
      type: String,
      max: 600,
