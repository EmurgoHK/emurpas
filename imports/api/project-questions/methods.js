...
export const notifyApplication = (type, resId, fieldId, text) => {
  ...
  if (application) {
    ...
    users.forEach(i => {
      // notify all users associated with the application
      sendNotification(
        i._id,
        `New question on your application (${resId}): ${text} (${fieldId})`,
        "System",
        `/applications/${application._id}/view`
      );
    });
  }
};
      ...
      if (Meteor.userId()) {
        if (projectID === "new") {
          ...
          if (progress && progress.status === "completed") {
            throw new Meteor.Error("Error", `Can't edit finished forms`);
          }
