...
Template.viewApplication.events({
  ...
  "click .comment-show": function(event, templateInstance) {
    ...
    templateInstance.show.set(
      this.question.key,
      !templateInstance.show.get(this.question.key)
    );
  }
});
...
Template.viewApplication.helpers({
  ...
  showLine: function() {
    return (
      Template.instance().reply.get(this.question.key) ||
      Template.instance().show.get(this.question.key)
    );
  },
  ...
  applicationStatus: () => {
    ...
    if (application && application.team_members) {
      ...
      if (!isFormComplete) {
        ...
        return {
          message: "Waiting for team members to complete personal information",
          class: "secondary"
        };
      } else if (hasModQuestions) {
        return {
          message:
            "The reviewers have questions for you, please view your application to respond",
          class: "warning"
        };
