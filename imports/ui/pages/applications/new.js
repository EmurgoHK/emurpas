...
Template.newApplication.onCreated(function() {
  ...
  this.autorun(() => {
    ...
    let applicationExist = FormProgress.findOne({
      status: "in-progress",
      form_type: "project"
    });
    if (applicationExist) {
...
Template.newApplication.onRendered(function() {
  this.autorun(() => {
    $("textarea[name=" + BC_USE_RSN + "]")
      .parent()
      .hide();
    $("textarea[name=" + BC_REQUIRE_RSN + "]")
      .parent()
      .hide();
    ...
    const progress = FormProgress.findOne({
      form_type_id: FlowRouter.getParam("projectID")
    });
    if (progress !== undefined) {
      const project = ProjectQuestions.findOne({
...
Template.newApplication.events({
  "change form"(event, tpl) {
    ...
    if (!AutoForm.validateField(targetKey, formId)) return;
    ...
    saveProjectQuestions.call(
      { projectID: projectID, data: { [targetKey]: value }, steps: steps },
      (err, resp) => {
        ...
        if (!err) {
          ...
          if (err.reason === `Can't edit finished forms`) {
            notify(
              "This application is marked as completed and cannot be modified!",
              "error"
            );
          } else {
  ...
  submit(event, tpl) {
    ...
    saveProjectQuestions.call(
      { projectID: projectID, data: wizard.mergedData(), steps: steps },
      (err, resp) => {
        if (!err) {
          if (projectID !== resp) FlowRouter.setParams({ projectID: resp });
